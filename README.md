...
最快的方式:到線上版找到想要的字體,按「複製提示詞」。複製到的就是站上那張效果圖的**同一段提示詞**,把 `[你的標題]` 這類方括號欄位換成自己的內容即可。

模板本體是 `fonts.json` 的 `templates`(`pure` 純字、`app` 應用),網頁與每日產圖 pipeline 共用同一份——要調整提示詞就改那裡,兩邊不再各寫一份。其中幾句是效果的關鍵,別刪:

- 「筆畫本身必須是〈風格描述〉所描述的實際形態或技法構成,不能只是在制式字體外觀上貼一層材質貼皮」——少了它,模型只會拿制式黑體貼一層材質
- 「共 N 個字,一字不多一字不少」——鎖字數
- 繁體字形、不得錯字漏字或簡體
- 應用圖那段的構圖規則(要有真實輔助視覺與小字、不要做成置中對稱的簡報卡)

自己從頭寫的話,照這個骨架(站上首頁的「提示詞公式」區塊給的是同一份):

```
[主題與場景描述]
主標題:「____」(共 N 個字,一字不多一字不少)
字體風格:____(不只是名稱,要描述材質與做法)
主標題的筆畫本身必須是這個風格描述的實際形態或技法構成,不能只在制式字體外觀上貼一層材質貼皮
[版型、情緒、輔助視覺與小字]
```

另外站上每張圖都經過 Gemini 驗字、最多重生 3 次才留下,自己生一次不如預期屬正常,再跑一次。
...
線上版每個字體條目並列顯示「字/應用」兩張效果圖,並提供兩顆複製鈕,分別複製 `templates.pure` 與 `templates.app` 填好該字體風格描述後的完整提示詞。
...
重生 3 次仍不合格的字體,會被挪到佇列尾端、換下一筆繼續(當天 13:30、16:30 還有兩個補跑時段),不會讓單一筆卡住整條產線。生圖服務本身掛掉則直接讓 workflow 失敗,佇列原封不動。驗字那支 Gemini 呼叫遇到 5xx、429 或讀取逾時會重試三次(間隔 20、40 秒),連三次都不通才失敗——2026-09-02 就是它抖了一整天,三個時段全折在第一次呼叫。
